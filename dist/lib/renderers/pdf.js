...
var debug = require('debug')('renderer:pdf');
...
function renderPDF(context, template) {
  ...
  return html.render(context, template, options).then(function (htmlStringResult) {
    // pick options relevant to rendering PDFs
    var pdfOptions = _.pick(options, ['pageSize', 'orientation', 'pageWidth', 'pageHeight', 'marginLeft', 'marginRight', 'marginTop', 'marginBottom', 'footerRight', 'footerFontSize']);
    ...
    var pdfStream = wkhtmltopdf(htmlStringResult, pdfOptions);
